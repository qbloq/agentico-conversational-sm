...
const WATI_CACHE_DIR = path.join(__dirname, 'data/wati');
...
async function main() {
  ...
  fs.mkdirSync(WATI_CACHE_DIR, { recursive: true });
  ...
  for (const contact of contacts) {
    ...
    const cacheFile = path.join(WATI_CACHE_DIR, `${contact.phone}.json`);
    let messages: WatiMessage[] = [];
    ...
    if (fs.existsSync(cacheFile)) {
      ...
      try {
        messages = JSON.parse(fs.readFileSync(cacheFile, 'utf-8'));
        process.stdout.write(`${messages.length} msgs (cached)\n`);
      } catch (err) {
        console.error(`Error reading cache for ${contact.phone}:`, err);
      }
    } else {
      process.stdout.write(`  - ${contact.phone}... `);
      ...
      messages = await wati.getMessages(contact.phone);
      process.stdout.write(`${messages.length} msgs\n`);
      
      // Save to cache
      if (messages.length > 0) {
        fs.writeFileSync(cacheFile, JSON.stringify(messages, null, 2));
      }
    }
  ...
  // 3. Analyze
  try {
    const report = await analyzeConversations(conversations, ai);
