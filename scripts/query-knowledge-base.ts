...
async function queryKnowledge(query: string) {
  ...
  try {
    // 1. Find the store by its display name (using async iterator)
    let store;
    ...
    if (!store) {
      console.error(`❌ Could not find FileSearchStore with display name: ${STORE_DISPLAY_NAME}`);
      return;
    }
    ...
    // 2. Query Gemini with the provider abstraction
    console.log('📡 Calling Provider with File Search & Structured Output...');
...
`;
    ...
    console.error('❌ Error querying knowledge base:', error.message || error);
  }
}

// Get query from command line arguments or use default
const userQuery = process.argv.slice(2).join(' ') || 'cuanto es el deposito minimo';
queryKnowledge(userQuery);
